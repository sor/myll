...
3. **Mutating code generation** — `NewExpr.Gen()` used to mutate the AST (`type.ptrs.RemoveAt(0)`). Fixed in `backend/Core/Expr.cs`; the method now temporarily replaces the pointer list and restores it. See `03-ast-core.md` and `plan/suspicious-warnings.md`.
4. **Dead/future code** — `Symbol.cs` and `Attribute.cs` enums exist but are disconnected. They are stubs intended for semantic analysis. See `07-future-stubs.md`.
5. **Local target-framework override** — The working tree may temporarily change `.csproj` and Rider `.run.xml` files from `net10.0` to `net6.0` to match a local SDK. Keep these changes unstaged and do **not** commit them; the repository target remains `net10.0`.
6. **Derived-to-base conversions, implicit aliases, and default attributes** — derived-to-base conversions are implemented for pointers, references, and smart pointers. Object slicing is rejected as a compile-time error in assignments, arguments, and returns; an explicit value cast that would slice now emits a warning instead of an error. Auto-unhiding of overloaded base methods is controlled by `Dialect.AutoUnhideBaseMethods` and can be overridden per class/method with `[shadow]` and `[unshadow]`. If a hidden name exists in multiple unrelated bases, auto-unhiding is skipped and a warning is emitted. `base` and `super` are not lexer keywords; `Dialect.BaseClassAliasName` (default `"base"`) creates an implicit alias for the first base class inside any class/struct that inherits. `Dialect.OwnClassAliasName` (default unset) can create an implicit alias for the enclosing class/struct. `self` is a reference to the current object (`(*this)`); `this` remains available as the pointer. `Dialect.AutoReturnName` (default `"ret"`) declares an implicit result variable for non-void functions. `Dialect.DefaultAttributesClass`/`Struct`/`Union`/`Enum` set per-kind default attributes applied before resolution. The built-in `(move)` and `(forward)` casts return the operand's own type and are emitted as `std::move(...)` and `std::forward(...)`. The literal `null` is implicitly convertible to raw and smart pointer types; it is rejected for value types and references. Unqualified access to base members from inside a derived method is not yet supported; use `object.baseMember()` syntax.
7. **Template overload resolution shortcuts** — basic validation, argument deduction for `T`/`T*`/`T&`/pointer wrappers, and `.template` disambiguation are implemented. A few cases are intentionally delegated to the C++ compiler: non-template class-template methods are accepted without full Myll-side argument conversion checks; constructor calls on external C++ types and builtin calls such as `static_assert` are not validated by Myll. Dependent nested types such as `T::Nested` and `Class<T>::Nested<U>` now generate the required C++ `typename` and `template` keywords; `testing/cases/template_typename_fail/` has been converted to a passing case and additional typename cases cover return types, parameters, variables, base classes, and nested template-ids.
8. **Global namespace `using`** — module-scope `using NS;` of a namespace is controlled by `Dialect.GlobalUsingNS`.
...
3. Implement reachable NotImplementedException features:
   ...
   - abstract/default/delete function bodies — DONE. `[abstract]` emits `= 0` and auto-virtual; `[default]` emits `= default`; `[delete]`/`[disallow]` emit `= delete`. See `testing/cases/abstract_default_delete/` and `frontend/apps/unix/ls.myll`.
    ...
    - pre-resolution transform migration — DONE. `AutoReturnTransformer`, `TemplateParamTransformer`, and `ChainTransformer` now run inside `backend/Resolver/Resolver.Resolve` instead of `frontend/Program.cs`. `[chain]` rejects explicit return types, warns on redundant trailing `return self;`, and cleaned up manual `-> Class&` / `return self;` boilerplate in `myll/dyn_array.myll`, `testing/cases/external_algorithms/main.myll`, and `frontend/tests/thesis/container.myll`.
    - post-resolution control-flow transform migration — DONE. `ElseOnLoopTransformer` and `BreakContinueTransformer` now run inside `backend/Resolver/Resolver.Resolve` post-resolution; removed from `frontend/Program.cs`.
    - post-resolution alias-shadowing transform migration — DONE. `ConfiguredAliasShadowingTransformer` now runs inside `backend/Resolver/Resolver.Resolve` post-resolution; removed from `frontend/Program.cs`. Rewrote it to walk the resolved scope tree via `CompilationContext.LocalDecls` instead of the broken `Stmt.EnumerateDF`, adding regression tests for loop-local and catch-parameter alias shadows.
    - prototype-file skip alignment — DONE. `ElseOnLoopTransformer` and `BreakContinueTransformer` now skip prototype files consistently with `AutoReturnTransformer` and `ChainTransformer`.
    ...
    - direct construction syntax — DONE. Variables can be declared with `var T name(args);` instead of `var T name = T(args);`. The parser accepts `funcCall` in `idAccessor`; the AST stores this explicitly via `VarDecl.isDirectConstruct` / `VarStmt.isDirectConstruct`. Empty argument lists emit `{}` instead of `()` to avoid the C++ most-vexing-parse issue. Direct construction works at module/field scope and inside functions, including namespaced template types. Regression case added at `testing/cases/direct_ctor/`.
    - initializer-list literals — DONE. `[1,2,3]` has the first-class type `initlist<T>` (alias `ilist<T>`), which lowers to C++ `std::initializer_list<T>`. `var auto a = [1,2,3];`, `var ilist a = [1,2,3];` and `var initlist<int> a = [1,2,3];` all work; empty lists are rejected when the element type must be inferred. Container targets such as `var std::vector<int> v = [1,2,3];` or `var Myll::DynArray<int> a = [1,2,3];` emit C++ brace-initialization and rely on the container's `std::initializer_list<T>` constructor. Auto-inferred `initlist` types are resolved so member access such as `var auto a = [1,2,3]; a.size()` works without an explicit import. Added `ctor initlist` (and `operator initlist =`) as special members: they automatically take a `const std::initializer_list<T>&` parameter and are marked `[implicit]`. The default ctor parameter name is `init`; the element type defaults to the class's first template parameter or can be given explicitly (and becomes a ctor/operator template parameter when it does not name a class template parameter). Added `std/std_initializer_list.decl.myll` and updated `myll/dyn_array.myll` to use `ctor initlist`. Standard C++ headers are no longer hardcoded in `StmtFormatting.DefaultIncludes`; they are emitted from the new `StdBuiltins.ImplicitStdImports` list, which is imported implicitly into every module. Regression case added/updated at `testing/cases/initializer_list/`.
