...
1. **Finish ScopeStack / semantic analysis (endboss)** — substantial progress made. Per-module scope trees, the fixed-point resolver, qualified (`A::B::C`) static names, `using namespace` / `using Name`, cross-module namespace merging, ambiguity diagnostics, per-declaration access modifiers, member access resolution, and exact-match overload resolution are implemented. The C++ generator now consumes resolved names for types, scoped expressions, and member access. Remaining: full type checking (assignments, returns, implicit conversions, narrowing diagnostics), overload promotion/conversion ranks with clear ambiguity/no-match diagnostics, and moving semantic validation out of the generator. See `plan/semantic-analysis.md`, `backend/Core/Scope.cs`, `backend/Resolver/Resolver.cs`, `backend/Core/Symbol.cs`.
