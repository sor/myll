...
7. **Derived-to-base conversions** — implemented for pointers, references, and smart pointers. Object slicing is rejected as a compile-time error. Unqualified access to base members from inside a derived method is not yet supported; use `object.baseMember()` syntax.
