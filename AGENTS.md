...
7. **Derived-to-base conversions** — implemented for pointers, references, and smart pointers. Object slicing is rejected as a compile-time error in assignments, arguments, returns, and explicit conversions. Auto-unhiding of overloaded base methods is controlled by `Dialect.AutoUnhideBaseMethods` and can be overridden per class/method with `[shadow]` and `[unshadow]`. If a hidden name exists in multiple unrelated bases, auto-unhiding is skipped and a warning is emitted. Unqualified access to base members from inside a derived method is not yet supported; use `object.baseMember()` syntax. The generated C++ `base` alias is not currently exposed to Myll source.
