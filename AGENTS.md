...
9. **`using namespace` ordering bug** — `using NS;` where `NS` is a namespace declared in the same module produces invalid C++ because Myll emits `using namespace NS;` before the namespace definition. Workaround: place the namespace in a separate imported module, or use fully qualified `NS::name` access. See `testing/cases/new_in_myll/using_ns.myll` and `using_ns_helper.myll` for the current workaround.
