...
1. **Finish ScopeStack / semantic analysis (endboss)** — substantial progress made. Per-module scope trees, the fixed-point resolver, qualified (`A::B::C`) static names, `using namespace` / `using Name`, cross-module namespace merging, ambiguity diagnostics, per-declaration access modifiers, member access resolution, ranked overload resolution, and core type checking (assignments, initializers, returns, argument compatibility) are implemented. The C++ generator now consumes resolved names for types, scoped expressions, and member access. Remaining: broader conversion ranks beyond exact + promotion, operator-specific and boolean-context checks, and moving semantic validation out of the generator. See `plan/semantic-analysis.md`, `backend/Core/Scope.cs`, `backend/Resolver/Resolver.cs`, `backend/Core/Symbol.cs`.
