...
3. Implement reachable NotImplementedException features:
   ...
   - abstract/default/delete function bodies — DONE. `[abstract]` emits `= 0` and auto-virtual; `[default]` emits `= default`; `[delete]`/`[disallow]` emit `= delete`. See `testing/cases/abstract_default_delete/` and `frontend/apps/unix/ls.myll`.
    ...
    - initializer-list literals — DONE. `[1,2,3]` produces a `std::initializer_list<T>` using the common compatible element type from the existing `TypeResolver.CommonType` machinery. `var auto a = [1,2,3];` infers `std::initializer_list<int>`; explicit container targets such as `var std::vector<int> v = [1,2,3];` or `var Myll::DynArray<int> a = [1,2,3];` emit C++ brace-initialization and rely on the container's `std::initializer_list<T>` constructor. Empty lists are rejected for `auto`. Added `std/std_initializer_list.decl.myll` and an initializer-list constructor to `myll/dyn_array.myll`. Regression case added at `testing/cases/initializer_list/`.
